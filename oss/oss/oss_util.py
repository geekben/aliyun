...
def create_part_xml(part_msg_list = []):
    '''
    get information from part_msg_list and covert it to xml.
    part_msg_list has special format.
    '''
...
def create_delete_object_msg_xml(object_list = [], is_quiet = False, is_defult = False):
    '''
...
def get_upload_id(oss, bucket, object, headers = {}):
    '''
...
class DeleteObjectWorker(Thread):
    def __init__(self, oss, bucket, part_msg_list):
...
class UploadPartWorker(Thread):
    def __init__(self, oss, bucket, object, upoload_id, file_path, part_msg_list, uploaded_part_map):
        Thread.__init__(self)
        self.oss = oss 
        self.bucket = bucket
        self.object = object
...
class MultiGetWorker(Thread):
    def __init__(self, oss, bucket, object, file, start, end):
        Thread.__init__(self)
        self.oss = oss
        self.bucket = bucket 
        self.object = object
